...
export function joinWorldChannel(
  ...
  onEnemies: (states: object[], items?: [string, number, number, number][], zoneIdx?: number) => void,
  ...
  _worldChannel = supabase.channel('world:positions', { config: { presence: { key: myUserId } } })
    ...
    .on('broadcast', { event: 'enemies' }, ({ payload }) => {
      onEnemies(payload.states ?? [], payload.items, payload.zoneIdx);
    })
...
export function broadcastEvent(payload: Record<string, unknown>) {
  _worldChannel?.send({ type: 'broadcast', event: 'evt', payload });
}
...
export function broadcastEnemyState(states: object[], items?: [string, number, number, number][], zoneIdx?: number) {
  _worldChannel?.send({ type: 'broadcast', event: 'enemies', payload: { states, items, zoneIdx } });
}
